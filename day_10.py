input = '''777FJ.F|-|FFF-L-J7.7.F-7--L-7-L-LF-7-FFF.FLF7.FL7-J77F-7.L7-7.-F77F|FLJ7.L---7.7---J77F|.F|-J7FFF|7-F7-77.FF7|-7-F77F-.J7.777FF|F|-F77|-77.F
...
for m in range(7000):
    for i in range(2):
        #print(i)
        #print(path[0-i])
        if path[0-i][0] == 'S' or path[0-i][0] == '|':
            nextY = path[0-i][1]+1
            nextX = path[0-i][2]
            toAdd = [lines[nextY][nextX],nextY,nextX]
...
for i in range(len(lines)):
    printLine = ''
    #print(i)
    inRow = [x for x in path if x[1] == i]
    for j in range(len(lines[0])):
        if j in [k[2] for k in inRow]:
            printLine += [k[0] for k in inRow if k[2] == j][0]
        else:
            printLine += '.'
...
#     for j in range(len(lines[0])):
#         if j in [k[2] for k in inRow]:
#             letter = [k[0] for k in inRow if k[2] == j][0]
#             if letter == 'F':
#                 printLine1 += '   '
#                 printLine2 += ' ■■'
#                 printLine3 += ' ■ '
...
#     lastlastB = ' '
#     for j, b in enumerate(a):
#         if b == ' ' and lastB == '■' and lastlastB == ' ':
#             inOut = -inOut
#         if b == '.':
#             dotSet += [[inOut, i, j]]
#             lineReplace += str(inOut) if inOut == 1 else '0'
#         else:
#             lineReplace += b
#         lastlastB = lastB
#         lastB = b
#     withGapsAndReplace += [lineReplace]
# print(len([x for x in dotSet if x[0] == 1]))
...
for i in range(len(lines)):
    ...
    dotsAndLoop += [printLine]

for i, a in enumerate(dotsAndLoop):
    print(i,a)
...
for i, a in enumerate(dotsAndLoop):
    ...
    lastB = ' '
    for j, b in enumerate(a):
        if b in ['|','S'] or (b == '7' and lastB == 'L') or (b == 'J' and lastB == 'F'):
            inOut = -inOut
        if b == '.':
            dotSet += [[inOut, i, j]]
            lineReplace += str(inOut) if inOut == 1 else '0'
        ...
        if b != '-':
            lastB = b
    replaceDots += [lineReplace]
print(len([x for x in dotSet if x[0] == 1]))

for i, a in enumerate(replaceDots):
    print(i, a)

ones = 0
for a in replaceDots:
    for b in a:
        if b == '1':
            ones += 1
...
# for i, a in enumerate(withGapsAndReplace):
#     print(i, a)
...
#         for x in range(3):
#             for y in range(3):
#                 if 0 <= a[0]+y-1 < len(withGaps) and 0 <= a[1]+x-1 < len(withGaps[0]):
#                     if withGaps[a[0]+y-1][a[1]+x-1] != '#':
#                         if [[a[0]+y-1,a[1]+x-1]] not in dotSet:
#                             print([[a[0]+y-1,a[1]+x-1]])
#                             dotSet += [[a[0]+y-1,a[1]+x-1]]
#                             #print(dotSet)
...
#                     for x in range(3):
#                         for y in range(3):
#                             if 0 <= m[1]+x-1 < len(withGaps) and 0 <= m[2]+y-1 < len(withGaps[0]):  
#                                 try:
#                                     if withGaps[m[1]+x-1][m[2]+y-1] in [' ','.'] and [withGaps[m[1]+x-1][m[2]+y-1],m[1]+x-1,m[2]+y-1] not in dotSet:
#                                         dotSet += [[withGaps[m[1]+x-1][m[2]+y-1],m[1]+x-1,m[2]+y-1]]
#                                 except:
#                                     print(f'Error at {m[1]+x-1} {m[2]+y-1}')
#             # for m in dotSet:
#             #     for x in range(3):
#             #         for y in range(3):
#             #             # print(withGaps[m[1]+x-1][m[2]+y-1])
#             #             # print([withGaps[m[1]+x-1][m[2]+y-1],m[1]+x-1,m[2]+y-1])
#             #             # print(dotSet)
#             #             # if withGaps[m[1]+x-1][m[2]+y-1] in [' ','.'] and [withGaps[m[1]+x-1][m[2]+y-1],m[1]+x-1,m[2]+y-1] not in dotSet:
#             #             #     dotSet += [[withGaps[m[1]+x-1][m[2]+y-1],m[1]+x-1,m[2]+y-1]]
#             #                 #print(dotSet)
#             #print([x for x in dotSet if x[0] == '.'])
#             #print(i, j, b, len([x for x in dotSet if x[0] == '.']))
#             toAdd = [x for x in dotSet if x[0] == '.']
#             toAdd.sort()
#             #print(toAdd)
#             if toAdd not in dotSets:
#                 dotSets += [toAdd]
#                 print(f'Dot Set starting at {toAdd[0][1]} {toAdd[0][2]} of size {len(toAdd)}')


# for a in dotSets:
#     print(f'Dot Set starting at {a[0][1]} {a[0][2]} of size {len(a)}')