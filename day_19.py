input = '''fmz{x<2152:R,s>3520:A,a<3391:A,A}
...
workflows = [{
    'mechanism': y[0], 
    'evaluations': y[1].split(',')
    } for y in [x.split('{') for x in input.split('\n\n')[0].splitlines()]]

for a in workflows:
    print(a)

parts = [eval(y) for y in [x.replace('{','{"').replace('=','":').replace(',',',"') for x in input.split('\n\n')[1].splitlines()]]
...
for a in parts:
    currentMech = 'in'
    #print(a)
    while currentMech not in ['A','R']:
        for b in [x['evaluations'] for x in workflows if x['mechanism'] == currentMech][0]:
            #print(b)
            if b[-1] == '}':
                #print(b[:-1])
                currentMech = b[:-1]
                break
            compareValue = int(a[b[0]])
            #print(compareValue)
            if b[1] == '<':
                if compareValue < int(b[2:b.index(':')]):
                    #print(b[b.index(':')+1:])
                    currentMech = b[b.index(':')+1:]
                    break
            if b[1] == '>':
                if compareValue > int(b[2:b.index(':')]):
                    #print(b[b.index(':')+1:])
        ...
        #print()
    if currentMech == 'A':
        totalScore += a['x'] + a['m'] + a['a'] + a['s']

print(totalScore)

#part 2

## THIS WORKS FOR THE EXAMPLE BUT NOT THE ACTUAL DATA 
...
# def splitToNextMech(destination):
#     output = []
#     destinationToUse = destination.copy()
#     if destinationToUse['destination'] not in ['A','R']:
#         for b in [x['evaluations'] for x in workflows if x['mechanism'] == destinationToUse['destination']][0]:
#             #print(b)
#             destinationToAdd = destinationToUse.copy()
#             if b[-1] == '}':
#                 destinationToAdd.update({'destination':b[:-1]})
#                 output += [destinationToAdd]
#                 break

#             compareValues = destinationToUse[b[0]]

            
#             currentStart = compareValues[0]
#             currentEnd = compareValues[1]
#             divider = int(b[2:b.index(':')])

#             if b[1] == '<':
#                 newEnd = divider-1 if currentStart < divider < currentEnd else currentEnd if divider >= currentEnd else currentStart
#                 if newEnd != currentStart:
#                     destinationToAdd.update({b[0]:[currentStart,newEnd]})
#                     destinationToAdd.update({'destination':b[b.index(':')+1:]})
#                     output += [destinationToAdd]
#                     destinationToUse.update({b[0]:[newEnd+1,currentEnd]})

#             if b[1] == '>':
#                 newStart = divider+1 if currentStart < divider < currentEnd else currentStart if divider <= currentStart else currentEnd
#                 if newStart != currentEnd:
#                     destinationToAdd.update({b[0]:[newStart,currentEnd]})
#                     destinationToAdd.update({'destination':b[b.index(':')+1:]})
#                     output += [destinationToAdd]
#                     destinationToUse.update({b[0]:[currentStart,newStart-1]})
#         return(output)
#     else:
#         return destinationToUse
...
#         print(a['destination'])
#         print((a['x'][1] - a['x'][0] + 1)*(a['m'][1] - a['m'][0] + 1)*(a['a'][1] - a['a'][0] + 1)*(a['s'][1] - a['s'][0] + 1) )

#         print([x for x in workflows if x['mechanism'] == a['destination']])
#         nextMechs = splitToNextMech(a)
#         for a in nextMechs:
#             print(a)
#         print()
#         newDestinations += [x for x in nextMechs if x['destination'] not in ['A','R']]
#         acceptedAndRejected += [x for x in nextMechs if x['destination'] in ['A','R']]
...
# print(sum([(x['x'][1] - x['x'][0] + 1)*(x['m'][1] - x['m'][0] + 1)*(x['a'][1] - x['a'][0] + 1)*(x['s'][1] - x['s'][0] + 1) for x in acceptedAndRejected if x['destination'] == 'A']))


# answer = 0
# for a in acceptedAndRejected:
#     xRange = (a['x'][1] - a['x'][0] + 1) if a['destination'] == 'A' else 0
#     mRange = (a['m'][1] - a['m'][0] + 1) if a['destination'] == 'A' else 0
#     aRange = (a['a'][1] - a['a'][0] + 1) if a['destination'] == 'A' else 0
#     sRange = (a['s'][1] - a['s'][0] + 1) if a['destination'] == 'A' else 0
#     answer += (xRange)*(mRange)*(aRange)*(sRange)

# print(answer)


## PART 2 rewrite with ranges --- works but no idea why it does and above doesn't 🤷 
...
def splitToNextMech(destination):
    ...
    if destinationToUse['destination'] not in ['A','R']:
        for b in [x['evaluations'] for x in workflows if x['mechanism'] == destinationToUse['destination']][0]:
            ...
            compareValues = list(destinationToUse[b[0]])
            
            divider = int(b[2:b.index(':')])

            if b[1] == '<':
                newValues = [x for x in compareValues if x<divider]
                leftOver = [x for x in compareValues if x>=divider]
                destinationToAdd.update({b[0]:newValues})
                destinationToAdd.update({'destination':b[b.index(':')+1:]})
                output += [destinationToAdd]
                destinationToUse.update({b[0]:leftOver})
                #print(destinationToAdd)
                #print(destinationToUse)
            ...
            if b[1] == '>':
                ...
                output += [destinationToAdd]
                destinationToUse.update({b[0]:leftOver})
        print(destinationToUse['x'][-1:0], destinationToUse['m'][-1:0], destinationToUse['a'][-1:0], destinationToUse['s'][-1:0])
        return output
    else:
        return destinationToUse
...
for a in destinations:
    print(a['x'][-1:0],a['m'][-1:0],a['a'][-1:0],a['s'][0],a['s'][-1],len(a['s']),a['destination'])

answer = 0
for a in acceptedAndRejected:
    print(a['x'][-1:0],a['m'][-1:0],a['a'][-1:0],a['s'][0],a['s'][-1],len(a['s']),a['destination'])
    if a['destination'] == 'A':
        answer += len(a['x'])*len(a['m'])*len(a['a'])*len(a['s'])
