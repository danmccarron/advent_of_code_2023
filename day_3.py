input = '''.............65..................998.........453...................................845..773.........................307....527...........541
...
cleanlines = clean.splitlines()

entries = [x.split('.') for x in cleanlines]
entriesold = [x.split('.') for x in lines]

parts = []
notParts = []
answer = 0
for x in range(len(lines)):
    # for y in [y for y in lines[x].split('.') if y != '' and not y.isnumeric()]: 
    #     if ''.join(x for x in y if x.isnumeric()) != '':
    #         #answer += int(''.join(x for x in y if x.isnumeric()))
    #         print(int(''.join(x for x in y if x.isnumeric())))
    for y in [y for y in cleanlines[x].split('.') if y.isnumeric()]:        
        position = cleanlines[x].find(y)
        digits = len(y)
        #print([x,position,y,digits])
        part = False
        #print(lines[x-1][position:position+digits])
        #print(lines[x+1][position:position+digits])
        if x != 0:
            #print(lines[x-1][position:position+digits])
            if not all(a in '.' for a in lines[x-1][position:position+digits]):
                #print([x,position,y,digits])
                part = True
        if x != len(lines)-1:
            #print(lines[x+1][position:position+digits])
        ...
        if position != 0:
            #print(lines[x][position-1])
            if lines[x][position-1] != '.':
                #print([x,position,y,digits])
                part = True
                
            if x !=0:
                #print(lines[x-1][position-1])
                if lines[x-1][position-1] != '.':
                    #print([x,position,y,digits])
                    part = True
        ...
        if position+digits != len(lines[x]):
            #print(lines[x][position+digits])
            if lines[x][position+digits] != '.':
                #print([x,position,y,digits])
                part = True
                
            if x !=0:
                #print(lines[x-1][position+digits])
                if lines[x-1][position+digits] != '.':
                    #print([x,position,y,digits])
                    part = True
        ...
        cleanlines[x] = cleanlines[x][:position] + "."*digits + cleanlines[x][position+digits:]
        
# print(answer)
#print(len(parts))
# print(len(notParts))

# print(len(list(set(parts))))
# print(len(list(set(notParts))))

print(sum(parts))
# print(sum(notParts))
# print(sum(list(set(parts))))
# print(sum(list(set(notParts))))
...
for a in numbers:
    border = [[a[0],a[1]-1],[a[0],a[1]+a[3]]]
    for i in range(-1,a[3]+1):
        border += [[a[0]-1,a[1]+i]]
        border += [[a[0]+1,a[1]+i]]
    if [x for x in border if x in asterisks] != []:
        a += [x for x in border if x in asterisks]


done = []
answer2 = 0
for a in [a for a in numbers if len(a)==5]:
    if a not in done:
        print(a)
        b = [b for b in [a for a in numbers if len(a)==5] if b[4] == a[4] and a != b]
        if len(b)>0:
            print(b[0])
        if len(b)>0:
            print(a[2],b[0][2])
            answer2 += int(a[2])*int(b[0][2])
            done += b
print(answer2)